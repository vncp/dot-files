...
" vim-fugitive
" brings up a git diff of the staged version of the file side by side with the
...
" If a window is open that contains the searched buffer, open that instead of opening that buffer again in the current window
" This will prevent conflicts between windows
...
"FZF
" Search for lines in all the open buffers
...
autocmd BufEnter *.{js,jsx,ts,tsx,cpp,c,h} :syntax sync fromstart
autocmd BufLeave *.{js,jsx,ts,tsx,cpp,c,h} :syntax sync clear
